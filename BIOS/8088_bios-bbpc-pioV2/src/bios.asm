...
%endif ; TURBO_MODE
	...
	mov	al,e_boot		; boot the OS POST code, this also sends 00 to DMA Page Register for DMA0
	out	post_reg,al
...
%ifdef MACHINE_BBPC8088
	setloc	0C000h		; POST Entry Point modified from B000 for reduced size to allow XTIDE Tiny BIOS to be added at F8000-F8FFF BIOS now Starts at F9000
    %include	"8086Monitorv7.inc"		; Embed Nanocomp 8088 Monitor at 0xC000, Power on reset goes to NC Reset, then B for BIOS, Now with IO Port 0x040x and NeoKey keypad
%endif
...
; Set up Nanocomp NMI and INT3 table entries so we can debug using Nanocomp Monitor in BIOS and MS-DOS
...
;%define NMISR 0x815D      ; Nanocomp Monitor Interrupt Routine
;%define REGDISPSWI 0x8416 ; Nanocomp Monitor Software Interrupt Register display

  xor ax, ax  ; Clear ax
  mov ds, ax  ; Set DS to low memory
  mov [ds:NMIVECTOR], word regdispswi
...
; Initialize PIT (8254 timer)
	...
	out	pit_ch1_reg,al		; pulse every 15us
...
; initialize keyboard controller (8242), keyboard and PS/2 auxiliary device
	...
	out	post_reg,al
%ifdef AT_KEYBOARD
;	call	kbc_init Commented out for initial BIOS testing until we get PS/2 Controller - does HALT on error
%else ; AT_KEYBOARD
...
;=========================================================================
; Breadboard PC VGA Palette Register definitions
; Note:
;	There is a 900+ byte gap in ROM here
;-------------------------------------------------------------------------
%ifdef MACHINE_BBPC8088
%include	"palette.inc"		; Needed for BB PC INT 10
%endif
;=========================================================================
...
.bytes:
	...
	db	MODEL_SUB_BYTE  ; byte 3: submodel = 0 added as parameter from config.inc defaults to 00
	db	00h			    ; byte 4: release = 0
%ifdef SECOND_PIC
%ifdef AT_RTC
	db	01110100b		; byte 5: feature byte 1 Added Extended BIOS Flag with PS2_MOUSE
...
detect_rom_ext:
	...
	mov	dx,0C800h
;	mov	bx,0F800h
	mov	bx,0F900h       ; Extend scan for XT IDE BIOS located from F8000-F8FFF
%ifdef BIOS_SETUP
	call	get_config_a
...
.fd_read_retry:
	...
	int	13h
	jb	.fd_failed
	mov	al, 0xD1		; Floppy Disk Drive Parameters
	out	post_reg,al
	...
	jz	.check_signature		; jump if zero drives
	mov	al, 0xD2		; Read Boot Sector
	out	post_reg,al
...
.fd_failed:
	pop	cx
	mov	al, 0xDF		; Floppy Disk Failed
	out	post_reg,al
    mov	al, [fdc_last_error]    ;Last Error
	out	post_reg,al
    mov	al, [fdc_ctrl_status]   ;ST0
	out	post_reg,al
    mov	al, [fdc_ctrl_status+1] ;ST1
	out	post_reg,al
    mov	al, [fdc_ctrl_status+2] ;ST2 
	out	post_reg,al
    mov	al, [fdc_ctrl_status+3] ;Cylinder
	out	post_reg,al
    mov	al, [fdc_ctrl_status+4] ;Head
	out	post_reg,al
    mov	al, [fdc_ctrl_status+5] ;Record (sector)
	out	post_reg,al
    mov	al, [fdc_ctrl_status+6] ;Number Bytes
	out	post_reg,al
	loop	.fd_read_retry
...
start:
    ;    jmp     bioscseg:cold_start
         jmp     bioscseg:reset     ; Start the Nanocomp Monitor, use B for BIOS
